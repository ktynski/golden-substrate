...
\begin{tikzpicture}[
    ...
    stage/.style={circle,minimum size=42pt,thick,inner sep=1pt,align=center},
    norm/.style={stage,draw=accentBlue!75,fill=accentBlue!8},
    flip/.style={stage,draw=red!70,fill=red!8},
    arr/.style={->,thick,accentGray!80,line width=0.8pt,
                shorten >=3pt,shorten <=3pt},
    lab/.style={font=\footnotesize,text=accentGray},
    evlbl/.style={font=\footnotesize\itshape,text=red!70}
  ]
  \node[norm] (A) at (0,0)
    {\footnotesize outside\\[-1pt]\footnotesize out};
  \node[norm] (B) at (3,0)
    {\footnotesize corrugate\\[-1pt]\footnotesize $k_1$};
  \node[flip] (C) at (6,0)
    {\footnotesize self-\\[-1pt]\footnotesize intersect};
  \node[norm] (D) at (9,0)
    {\footnotesize corrugate\\[-1pt]\footnotesize $k_2, k_3$};
  \node[norm] (E) at (12,0)
    {\footnotesize inside\\[-1pt]\footnotesize out};

  \draw[arr] (A) -- node[above,lab] {$T_{\fP_5}$} (B);
  \draw[arr] (B) -- node[above,evlbl] {$T_{\fP_3}$} node[below,lab] {sign flip} (C);
  \draw[arr] (C) -- node[above,lab] {$T_{\fP_{31}}$} (D);
  \draw[arr] (D) -- node[above,lab] {resolve} (E);

  \draw[accentBlue,thick,decorate,
        decoration={snake,amplitude=1.5pt,segment length=7pt}]
        (-0.6,-1.4) -- (0.6,-1.4);
  \node[lab] at (0,-1.9) {$S^2$ (normal)};

  \draw[accentBlue,thick,decorate,
        decoration={snake,amplitude=3pt,segment length=4pt}]
        (2.4,-1.4) -- (3.6,-1.4);
  \node[lab] at (3,-1.9) {corrugated};

  \draw[red!70,thick] (5.5,-1.1) .. controls (5.8,-1.7) and (6.2,-1.7) ..
                       (6.5,-1.1);
  \draw[red!70,thick] (5.5,-1.7) .. controls (5.8,-1.1) and (6.2,-1.1) ..
                       (6.5,-1.7);
  \node[lab,text=red!70] at (6,-2.1) {Morin surface};

  \draw[accentBlue,thick,decorate,
        decoration={snake,amplitude=2.5pt,segment length=3pt}]
        (8.4,-1.4) -- (9.6,-1.4);
  \node[lab] at (9,-1.9) {multi-freq.};

  \draw[accentBlue,thick] (12,-1.4) circle (0.5);
  \draw[accentBlue,thick,->] (12,-0.9) -- (12,-0.6);
  \node[lab] at (12,-2.1) {$S^2$ (everted)};

  \node[lab,font=\footnotesize\itshape] at (6,-2.9)
    {Sphere eversion as Hecke propagation:
     $\psi_k = T_{\fP_k}\psi_{k-1}$,\quad
     eversion completes when $\prod a_{\fP_k} < 0$};
\end{tikzpicture}
\caption{Schematic of sphere eversion realized as Hecke propagation.
The five stages correspond to: (1)~the initial sphere with outward
normal; (2)~corrugation at frequency $k_{\fP_5}=5$ (ramified prime);
(3)~the Morin-type self-intersection where the cuspidal sign flips
($a_{\fP_3}=-5$, inert prime); (4)~multi-frequency corrugation after
the split prime $\fP_{31}$; (5)~the everted sphere with reversed
orientation.  Each arrow is a single Hecke step
$\psi_k = T_{\fP_k}\psi_{k-1}$.  The sign flip at stage~3 is the
arithmetic realization of Thurston's topological eversion.}
\label{fig:eversion-schematic}
\end{figure}
...
Propagation through the $\Cl(3,1)$ Substrate} (Paper~VII;
this paper), 2026.
