...
substrate of Paper~I~\cite{paper-I}. The contribution is the bijection, the
...
main trilogy (I~$\to$~II~$\to$~III).  It depends on Paper~I~\cite{paper-I}
(Clifford algebra, Peirce idempotents) and Paper~III~\cite{paper-III} (monad/braid
...
unique algebraic realization in the substrate of Paper~I~\cite{paper-I}. The
...
Lyapunov property of Paper~I~\cite{paper-I}, is exactly one. The bijection is
forced by:
...
This paper depends on Paper~I~\cite{paper-I} for the algebraic substrate
($\Cl(3,1)$, the Grace contraction, the idempotent split). It does
not depend on Papers~II--IV~\cite{paper-II,paper-III,paper-IV}. The metabolic reading is a new
...
We assume the reader is familiar with the substrate of Paper~I~\cite{paper-I} and
...
Paper~I~\cite{paper-I}, with $\Pf + \Qf = I$ and $\Pf\Qf = \Qf\Pf = 0$. In the
...
fixed in Paper~I~\cite{paper-I}) $c_i \mapsto e^{-(1 - \vp^{-\mathrm{grade}(e_i)})} c_i$
...
matrix-level update of Paper~III~\cite{paper-III} \S2. $S$ preserves the first matrix
...
\begin{tikzpicture}[
    ...
    phase/.style={circle,draw=accentBlue!75,fill=accentBlue!10,
                  thick,minimum size=42pt,inner sep=2pt,align=center},
    arr/.style={->,very thick,accentGray!85},
    lab/.style={font=\footnotesize,text=accentGray}
  ]
  \node[phase] (O) at (90:2.4)  {$\Omega$\\[-1pt]{\scriptsize intake}};
  \node[phase] (L) at (0:2.4)   {$L$\\[-1pt]{\scriptsize lysis}};
  \node[phase] (W) at (270:2.4) {$W$\\[-1pt]{\scriptsize witness}};
  \node[phase] (G) at (180:2.4) {$G$\\[-1pt]{\scriptsize growth}};

  \draw[arr] (O) arc[start angle=90,end angle=10,radius=2.4]
    node[lab,pos=0.5,right=4pt] {decompose};
  \draw[arr] (L) arc[start angle=0,end angle=-80,radius=2.4]
    node[lab,pos=0.5,below right=2pt] {residue};
  \draw[arr] (W) arc[start angle=270,end angle=190,radius=2.4]
    node[lab,pos=0.5,left=4pt] {absorbed};
  \draw[arr] (G) arc[start angle=180,end angle=100,radius=2.4]
    node[lab,pos=0.5,above left=2pt] {mixed};

  \node[font=\footnotesize\bfseries,text=accentBlue!80] at (0,0)
    {$R = G(W(L(\Omega)))$};
\end{tikzpicture}
\caption{The metabolic cycle $R = G(W(L(\Omega)))$ as a circular flow.
External matter $\Omega$ is decomposed (lysis $L$), the
boundary-preserving residue is witnessed ($W$), and Grace
contraction ($G$) stabilises the absorbed product.  The labels
on each arc indicate the algebraic product at that stage:
\emph{mixed} (ingested sum), \emph{decompose} (grade
decomposition), \emph{residue} (excreted complement), and
\emph{absorbed} (Grace-stable projection).}
...
\begin{tikzpicture}[
  ...
  \draw[->,thick] (0,-0.3) -- (0,3.8) node[above] {$c_k(t)/c_k(0)$};
  \foreach \y/\lab in {1/0.5, 2/1.0} {
    \draw[accentGray!40] (0,\y*3) -- (7,\y*3);
    \draw (-0.1,\y*3) -- (0.1,\y*3) node[left=4pt,font=\footnotesize] {\lab};
  }
  \foreach \x in {2,4,6} {
    \draw (\x,-0.1) -- (\x,0.1) node[below=2pt,font=\footnotesize] {\x};
  }
  \draw[accentBlue!90, very thick]
    plot[domain=0:7,samples=60] (\x, {3});
  \node[right,font=\footnotesize,accentBlue!90] at (7.1,3) {$k{=}0$};
  \draw[accentBlue!70, thick, dashed]
    plot[domain=0:7,samples=60] (\x, {3*exp(-\x*0.382)});
  \node[right,font=\footnotesize,accentBlue!70] at (7.1,{3*exp(-7*0.382)}) {$k{=}1$};
  \draw[accentGray!80, thick]
    plot[domain=0:7,samples=60] (\x, {3*exp(-\x*0.618)});
  \node[right,font=\footnotesize,accentGray!80] at (7.1,{3*exp(-7*0.618)}) {$k{=}2$};
  \draw[red!60!black, thick, densely dotted]
    plot[domain=0:7,samples=60] (\x, {3*exp(-\x*0.764)});
  \node[right,font=\footnotesize,red!60!black] at (7.1,{3*exp(-7*0.764)}) {$k{=}3$};
  \draw[red!80, thick, dashdotted]
    plot[domain=0:7,samples=60] (\x, {3*exp(-\x*0.854)});
  \node[right,font=\footnotesize,red!80] at (7.1,{3*exp(-7*0.854)}) {$k{=}4$};
\end{tikzpicture}
\caption{Grace decay curves $c_k(t)/c_k(0) = \exp(-t(1-\vp^{-k}))$ for
grades $k=0,\ldots,4$.  The scalar ($k{=}0$) is the unique fixed point;
the pseudoscalar ($k{=}4$) decays fastest with rate $1-\vp^{-4}\approx 0.854$.
The five decay rates are $\{0, 1{-}\vp^{-1}, 1{-}\vp^{-2}, 1{-}\vp^{-3}, 1{-}\vp^{-4}\}$, fully determined by the golden ratio.}
\label{fig:grace-decay}
\end{figure}
...
golden-ratio scaling fixed in Paper~I~\cite{paper-I}.
\end{remark}
...
Let $S$ be the master step of Paper~III~\cite{paper-III} and $G$ the Grace
contraction. Then:
...
representation (Paper~III~\cite{paper-III} \S2). For (iii): the trace
...
\item \emph{Constipation.} Excretion blocked.   From the throat
  architecture of Paper~IV~\cite{paper-IV}, if $P_\mathrm{clear} = 0$
...
in $\{0, 1\}$ for $g$ and ranges from Paper~III~\cite{paper-III} \S3. For each, run
...
machine-verified Promotion Lemma of Paper~III~\cite{paper-III}. The two proofs
agree.
\end{remark}
...
the substrate of Paper~I~\cite{paper-I} or is a uniformly random test input drawn
...
the FST) appears in Papers~I--IV~\cite{paper-I,paper-II,paper-III,paper-IV}. This paper introduces no new
...
\bibitem{paper-I}
  K.~Tynski,
  \emph{The Golden Substrate: From Null Generators to the Split Albert Algebra},
  Paper~I in the Golden Substrate series, May 2026.

\bibitem{paper-II}
  K.~Tynski,
  \emph{Echo Interference Implies the Riemann Hypothesis and the Collatz Conjecture},
  Paper~II in the Golden Substrate series, May 2026.

\bibitem{paper-III}
  K.~Tynski,
  \emph{The Holographic Monad: Why the Substrate Proves Universal Theorems},
  Paper~III in the Golden Substrate series, May 2026.

\bibitem{paper-IV}
  K.~Tynski,
  \emph{$\Z[\vp]$ Arithmetic in the Monstrous Atlas},
  Paper~IV in the Golden Substrate series, May 2026.

\bibitem{paper-VI}
  K.~Tynski,
  \emph{The Operator-Level Bridge},
  Paper~VI in the Golden Substrate series, May 2026.
