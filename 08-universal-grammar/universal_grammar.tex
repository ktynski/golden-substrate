...
\newcommand{\seamfix}{\mathrm{seam\_fix}}
\newcommand{\seamtwist}{\mathrm{seam\_twist}}
...
\begin{tabular}{|c|}
\hline
\textbf{Layer 0:} $G_2 = \Aut(\OO)$, dimension $14$, exceptional \\
$\downarrow$ \\
\textbf{Layer 1:} Fano plane $\PG(2,2)$: 7 points, 7 lines \\
$\downarrow$ \\
\textbf{Layer 2:} $\Cl(3)$ spatial bulk: $8 = 1+3+3+1$ \\
$\downarrow$ \\
\textbf{Layer 3:} $(\Cl(3,1), \Cl(1,3), \text{seam})$: eversion law \\
$\downarrow$ \\
\textbf{Layer 4:} Witness $= (\mathsf{be}, \mathsf{know})$: unique
                   grade-$0$/$4$ Hodge pair \\
$\downarrow$ \\
\textbf{Layer 5:} 21 blade primes (forced by bipolar/monopole partition) \\
$\downarrow$ \\
\textbf{Layer 6:} 9 channel primes (4 fundamental projectors) \\
$\downarrow$ \\
\textbf{Layer 7:} 11 operator primes (9 unary + 2 binary) \\
$\downarrow$ \\
\textbf{Layer 8:} Universal Grammar theorem \\
\hline
\end{tabular}
\caption{The eight-layer derivation chain.  Each layer is forced
by the one above; each carries an executable verification probe;
every probe passes.}\label{fig:chain}
\end{figure}
...
Section~\ref{sec:layer0}--\ref{sec:layer7} present each of the
...
We verify two closure properties: (i) every NSM spatial prime
...
universal.  We test $10$ of the $\sim 47$ Greenberg universals in
