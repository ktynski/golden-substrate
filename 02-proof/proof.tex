...
{\large Paper II in the Golden Substrate series}}
...
\begin{abstract}
\noindent
This is Paper II in the Golden Substrate series.  Paper I established the
...
(\cref{thm:supercoiling-lemma}) shows that any non-capping Collatz braid
...
\begin{itemize}[leftmargin=1.5em,nosep,topsep=2pt]
  \item Defines the \emph{echo propagator} $\EF_\vp$ on the
...
%% =====================================================================
%% =====================================================================
%% PART A --- INTRODUCTION AND SUBSTRATE SUMMARY
%% =====================================================================
\part*{Part A --- Introduction and Substrate Summary}
\addcontentsline{toc}{part}{Part A --- Introduction and Substrate Summary}
...
\textbf{Part A} (this introduction and \cref{sec:substrate-summary})
...
\textbf{Part B} (\cref{sec:propagator,sec:compatibility-rh,sec:six-traps})
...
\textbf{Part C} (\cref{sec:route-b,sec:route-c,sec:predictions,sec:collatz})
...
\textbf{Part D} (\cref{sec:lean,sec:27-formulations,sec:status,sec:discussion})
...
Parts B--D depend on.  All proofs and detailed constructions
...
%% =====================================================================
%% PART B --- ROUTE A: THE PROOF
%% =====================================================================
\part*{Part B --- Route A: The Riemann Hypothesis Proof}
\addcontentsline{toc}{part}{Part B --- Route A: The RH Proof}

%% =====================================================================
\section{The Echo Propagator $\EF_\vp$}
...
not a spectral realization: the echo propagator $\EF_\vp$ does not
...
with light reframing inside the substrate of Part A.  The
...
What we do instead is define a single graded operation $\EF_\vp$ on
...
\begin{enumerate}
  \item $\EF_\vp$ fixes the prime-side Dirichlet series
...
\begin{tikzpicture}[
  node distance=1.8cm,
  stepbox/.style={
    rectangle, rounded corners=3pt, draw=accentBlue!70, thick,
    fill=accentBlue!6, text width=11em, minimum height=2.6em,
    align=center, font=\small},
  arr/.style={-{Stealth[length=5pt]}, thick, accentBlue!80}
]
\node[stepbox] (s1) {Grade-0 fixes\\Dirichlet coefficients};
\node[stepbox, right=2.2cm of s1] (s2) {AC bridge: coefficients\\determine meromorphic $F$};
\node[stepbox, right=2.2cm of s2] (s3) {Grade-4 contraction:\\$\rho \mapsto \rho'$};
\node[stepbox, below=1.4cm of $(s2)!0.5!(s3)$] (s4)
  {Compatibility:\\$\rho' = \rho$ forces $\Re(\rho)=\tfrac{1}{2}$};
\draw[arr] (s1) -- (s2);
\draw[arr] (s2) -- (s3);
\draw[arr] (s2.south) -- ++(0,-0.5) -| (s4.north west);
\draw[arr] (s3.south) -- ++(0,-0.5) -| (s4.north east);
\end{tikzpicture}
\caption{The four-step architecture of the Route~A proof.  Grade-0
fixation and the analytic-continuation bridge determine the
meromorphic function and its poles; grade-4 contraction prescribes
a shifted pole $\rho'$; compatibility of both actions within a
single operator forces $\rho'=\rho$ for every nontrivial zero.}
...
(Remark~3.7 in Paper~I \cite[Rmk.~3.7]{paper-I-substrate}).
...
The echo propagator $\EF_\vp$ acts on $\lambda(\rho) = s_0 + s_4 I$ by
\begin{equation}
  \EF_\vp^n(s_0 + s_4 I) \;=\; s_0 + \vp^{-4n} s_4 I.
  \label{eq:propagator}
\end{equation}
...
emitted by the propagator $\EF_\vp$ on the grade-$4$ pseudoscalar
...
\begin{tikzpicture}[
  scale=1.1,
  >=Stealth,
  zero/.style={circle, fill=accentBlue, inner sep=1.8pt},
  shifted/.style={circle, fill=red!70!black, inner sep=1.8pt},
  oncrit/.style={circle, fill=accentBlue!60!green, inner sep=2pt},
]
\draw[->] (-0.4,0) -- (3.6,0) node[right] {$\Re(s)$};
\draw[->] (0,-0.6) -- (0,4.2) node[above] {$\Im(s)$};

\draw[accentBlue, thick, dashed] (1.25,-0.4) -- (1.25,4.0)
  node[above, font=\footnotesize] {$\Re(s)=\tfrac{1}{2}$};

\node[zero, label={[font=\footnotesize]right:$\rho$ (off-critical)}] (rho)
  at (2.2,2.6) {};
\node[shifted, label={[font=\footnotesize]left:$\rho'$ (shifted)}] (rhop)
  at (1.7,2.55) {};
\draw[->, thick, red!70!black, shorten >=2pt, shorten <=2pt]
  (rho) -- (rhop);

\node[oncrit, label={[font=\footnotesize]right:$\rho=\rho'$}] (oncrit)
  at (1.25,1.3) {};

\node[font=\footnotesize, text=accentGray, text width=6em, align=center]
  at (2.9,1.0) {on-critical:\\$s_4=0$};
\node[font=\footnotesize, text=accentGray, text width=7.5em, align=center]
  at (3.0,3.2) {off-critical:\\$\rho'\neq\rho$};
\end{tikzpicture}
\caption{Pole-shift geometry in the complex $s$-plane.  The critical
line $\Re(s)=\frac{1}{2}$ is shown dashed.  For a hypothetical
off-critical zero $\rho$, the grade-4 contraction maps
$\rho\mapsto\rho'\neq\rho$ (red arrow).  On the critical line,
$s_4=0$ so $\rho'=\rho$ and the shift is trivial (green dot).
Compatibility of both grades forces every nontrivial zero into
the on-critical case.}
\label{fig:pole-shift-geometry}
\end{figure}
...
the grade-$0$ fixation $\EF_\vp[P] = P$ (an arithmetic statement
...
corrections.  The propagator $\EF_\vp$ is a single graded
...
The two must agree:
\[
  F(s) \;=\; \EF_\vp[F](s) \;=\; \sum_\rho \frac{1}{s - \rho'(\rho)} + R(s),
\]
...
\begin{enumerate}
  \item the propagator's grade-$0$ eigenvalue is $1$ (an
    \emph{intrinsic} fact about $\EF_\vp$, from
  ...
  \item the propagator's grade-$4$ eigenvalue is $\vp^{-4}$ (an
    \emph{intrinsic} fact about $\EF_\vp$, from the zero-defect
...
The echo propagator $\EF_\vp$ is not in this position.  Its grade
...
propagator $\EF_\vp$ is a graded operation on the explicit-formula
...
The proof does not assert ``$\EF_\vp$ preserves $P = Z$ because it
...
In the substrate language of Part A, the echo propagator's
...
forward connection; the explicit identification of $\EF_\vp$ with a
specific generator of $\Der(J_3(\Os)) \cong \fk{f}_{4(4)}$ is a
natural next step, but is not undertaken in the present paper.
\end{remark}

%% =====================================================================
%% PART C --- CONDITIONAL ALTERNATES AND PREDICTIONS
%% =====================================================================
\part*{Part C --- Conditional Alternates and Predictions}
\addcontentsline{toc}{part}{Part C --- Conditional Alternates and Predictions}
...
\input{collatz_and_master.tex}

%% =====================================================================
%% PART D --- FORMALIZATION, ENUMERATION, DISCUSSION
%% =====================================================================
\part*{Part D --- Formalization, Enumeration, Discussion}
\addcontentsline{toc}{part}{Part D --- Formalization, Enumeration, Discussion}
...
\begin{enumerate}
  ...
  \item A formal definition of how $\EF_\vp$ acts on a meromorphic
...
\item Route A: echo-propagator $\EF_\vp$ compatibility (RH proof).
...
\midrule
\multicolumn{2}{l}{\textbf{Substrate (Part A)}} \\
...
\midrule
\multicolumn{2}{l}{\textbf{Compatibility law (Part A)}} \\
...
\midrule
\multicolumn{2}{l}{\textbf{Route A (Part B)}} \\
...
\midrule
\multicolumn{2}{l}{\textbf{Conditional alternates (Part C)}} \\
...
\midrule
\multicolumn{2}{l}{\textbf{Experimental (Part D)}} \\
...
runs on the substrate via the echo propagator $\EF_\vp$ with
...
on which all of Part A rests---the QSNV null substrate, the
