...
{\large Paper III in the Golden Substrate series}}
...
\begin{abstract}
\noindent
This is Paper III in the Golden Substrate series.  Paper~I established
...
This is Paper III in the Golden Substrate series.
...
\begin{tikzpicture}[
    ...
    paper/.style={rectangle, draw=accentBlue!75, fill=accentBlue!8,
                  thick, rounded corners=3pt, minimum width=2.0cm,
                  minimum height=0.8cm, align=center},
    dep/.style={->, thick, accentGray!80},
  ]
  %% Core trilogy
  \node[paper, draw=red!60, fill=red!8] (P1) at (0, 0) {I\\[-2pt]\scriptsize Substrate};
  \node[paper, draw=red!60, fill=red!8] (P2) at (3, 0) {II\\[-2pt]\scriptsize Proof};
  \node[paper, draw=red!60, fill=red!8] (P3) at (6, 0) {III\\[-2pt]\scriptsize Monad};

  %% Extended series
  \node[paper] (P4) at (0, -1.8) {IV\\[-2pt]\scriptsize Moonshine};
  \node[paper] (P5) at (3, -1.8) {V\\[-2pt]\scriptsize Metabolic};
  \node[paper] (P6) at (6, -1.8) {VI\\[-2pt]\scriptsize Operator};
  \node[paper] (P7) at (1.5, -3.6) {VII\\[-2pt]\scriptsize Eversion};
  \node[paper] (P8) at (4.5, -3.6) {VIII\\[-2pt]\scriptsize Grammar};

  %% Core arrows
  \draw[dep] (P1) -- (P2);
  \draw[dep] (P2) -- (P3);

  %% From I to extensions
  \draw[dep] (P1) -- (P4);
  \draw[dep] (P1) -- (P5);
  \draw[dep] (P1) -- (P6);

  %% From II/III to extensions
  \draw[dep] (P2) -- (P5);
  \draw[dep] (P3) -- (P6);

  %% From earlier extensions to later
  \draw[dep] (P4) -- (P7);
  \draw[dep] (P6) -- (P7);
  \draw[dep] (P6) -- (P8);
  \draw[dep] (P5) -- (P8);
  ...
  %% Legend
  \node[font=\footnotesize, text=accentGray, anchor=north west] at (-1.2, -4.8)
    {Highlighted: core trilogy (I\,$\to$\,II\,$\to$\,III).
     Arrows denote prerequisite dependencies.};
\end{tikzpicture}
\caption{Dependency graph of the Golden Substrate series (Papers
I--VIII).  Paper~I (Substrate) is the common foundation; Papers~II
and III form the core trilogy.  Papers~IV--VIII extend the framework
into moonshine arithmetic, metabolic interpretation, operator-level
bridges, sphere eversion, and universal grammar.  Arrows indicate
prerequisite reading order.}
\label{fig:series-dependency}
\end{figure}
...
\begin{keyresultbox}
\begin{proposition}[Scalar shadow --- the categorical reading of the no-residue parent axiom]
\label{prop:shadow}
...
\begin{tikzpicture}[
    >=Latex,
    every node/.style={font=\small},
    block/.style={rectangle, draw=accentBlue!75, fill=accentBlue!8,
                  thick, rounded corners=2pt, minimum width=5cm,
                  minimum height=0.7cm, align=center},
    wt/.style={font=\footnotesize, text=accentGray},
    sumline/.style={thick, accentBlue!60, dashed}
  ]
  \def\dy{1.15}
  \node[block] (b0) at (0, 0)        {Block $n=0$};
  \node[wt]    at (3.4, 0)           {$\vp^{0} = 1$};
  \node[block] (b1) at (0, -\dy)     {Block $n=1$};
  \node[wt]    at (3.4, -\dy)        {$\vp^{-2}$};
  \node[block] (b2) at (0, -2*\dy)   {Block $n=2$};
  \node[wt]    at (3.4, -2*\dy)      {$\vp^{-4}$};
  \node[block] (b3) at (0, -3*\dy)   {Block $n=3$};
  \node[wt]    at (3.4, -3*\dy)      {$\vp^{-6}$};
  \node[font=\large, text=accentGray] at (0, -3.9*\dy) {$\vdots$};
  \node[wt]    at (3.4, -3.9*\dy)    {$\vdots$};
  \node[block, draw=red!60, fill=red!8] (bN) at (0, -5*\dy)
    {Block $n \to \infty$};
  \node[wt]    at (3.4, -5*\dy)      {$\vp^{-2n} \to 0$};
  %% Arrows between blocks
  \draw[->, thick, accentGray] (b0) -- (b1);
  \draw[->, thick, accentGray] (b1) -- (b2);
  \draw[->, thick, accentGray] (b2) -- (b3);
  %% Summation brace on the left
  \draw[decorate, decoration={brace, amplitude=6pt, mirror},
        thick, accentBlue!70]
    (-3.1, 0.4) -- (-3.1, -5*\dy-0.4)
    node[midway, left=10pt, align=right, font=\small, text=accentBlue]
    {$\displaystyle\sum_{n=0}^{\infty}\vp^{-2n} = \vp$};
\end{tikzpicture}
\caption{The Apollonian recursion tower.  Each block applies the
same closure law $D(\vp) = 0$, weighted by $\vp^{-2n}$ at depth~$n$.
The geometric series converges:
$\sum_{n=0}^{\infty}\vp^{-2n} = \vp^2/(\vp^2 - 1) = \vp$.
The fixed point of the recursion is the golden ratio itself---the
tower's total contribution equals the unit it partitions.  Blocks
are identical in law; only the Apollonian scale differs.}
\label{fig:apollonian-tower}
\end{figure}
...
\begin{remark}[Grade-1 eigenvalue: theoretical spectrum vs.\ pinned spectrum]
\label{rmk:grade1-discrepancy}
Equation~\eqref{eq:grace-spectrum} gives the \emph{theoretical}
echo-propagator spectrum $\vp^{-g}$, which assigns grade-$1$ the
eigenvalue $\vp^{-1} \approx 0.618$.  By contrast, the SpineV3/V6
implementations (\cref{sec:spine-v3,sec:spine-v6}) and the
EchoNorm tuples reported earlier in this paper (e.g.\
$(1, 1, \rho^{-2}, \rho^{-3}, \rho^{-4})$) \emph{pin} the grade-$1$
eigenvalue to $1$, the same as grade-$0$.  The pinning reflects an
architectural choice: in the LM realization, grade-$1$ (vector /
explicit semantic content) must pass through the block without
attenuation so that token-level information survives the residual
stream.  The theoretical spectrum $\vp^{-g}$ describes the
\emph{intrinsic} scale hierarchy of the closure law; the pinned
spectrum $(1, 1, \vp^{-2}, \vp^{-3}, \vp^{-4})$ is the
\emph{implementable} variant that preserves the information channel
while respecting the same relative ordering for grades $\ge 2$.
\end{remark}
...
\subsection{Braid-knot trace and the supercoiling lemma}
...
The mathematical content of the Golden Substrate series is, taken
...
\bibitem{paper-IV-moonshine}
K.~Tynski,
\emph{$\mathbb{Z}[\varphi]$ Arithmetic in the Monstrous Atlas:
Moonshine Scaffolding for the Golden Substrate Seam},
Paper~IV in this series,
\texttt{papers/04-moonshine-context/moonshine\_context.tex}, May 2026.

\bibitem{paper-V-metabolic}
K.~Tynski,
\emph{The Metabolic Interpretation of FSCTF: Eating as the Creation
Cycle}, Paper~V in this series,
\texttt{papers/05-metabolic/metabolic.tex}, May 2026.

\bibitem{paper-VI-operator}
K.~Tynski,
\emph{The Operator-Level Bridge: From Peirce Decomposition to Prime
Splitting via the Quaternion-Clifford Identification},
Paper~VI in this series,
\texttt{papers/06-operator-bridge/operator\_bridge.tex}, May 2026.

\bibitem{paper-VII-eversion}
K.~Tynski,
\emph{The Information-Acoustic Eversion: Sphere Eversion as Hecke
Propagation through the $\mathrm{Cl}(3,1)$ Substrate},
Paper~VII in this series,
\texttt{papers/07-eversion/eversion.tex}, May 2026.

\bibitem{paper-VIII-grammar}
K.~Tynski,
\emph{From $G_2$ to Universal Grammar: A Math-First Derivation of
Natural Semantic Metalanguage via $\mathrm{Cl}(3,1)$},
Paper~VIII in this series,
\texttt{papers/08-universal-grammar/universal\_grammar.tex}, May 2026.
