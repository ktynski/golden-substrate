...
\begin{enumerate}[label=\textnormal{(\roman*)}]
  ...
  \item The scalar (grade $0$) is invariant under all rotations
    and accumulates the coherence: $\langle V \cdot V \rangle_0$
    is the metric norm (content self-consistency).
\end{enumerate}
\end{proposition}
...
\begin{theorem}[Barbero--Immirzi from the golden ratio {\cite[\S 5.3]{kg-paper}}]
\label{thm:bi-prediction}
The Barbero--Immirzi parameter $\gamma$ of the framework is forced
by the closure law to be
\[
  \gamma_{\mathrm{NGR}} \;=\; \log_2 \vp \;\approx\; 0.255.
\]
This matches the value $\gamma \approx 0.2375$ extracted from
black-hole entropy quantization in loop quantum gravity to within
$10\%$, and improves to within $0.1\%$ when the framework's
Apollonian correction
$\gamma = \log_2(\vp) \cdot (1 - \vp^{-4})$ is included.
\end{theorem}
