...
\newcommand{\Zp}{\ensuremath{\mathbb{Z}[\varphi]}}
\newcommand{\Qp}{\ensuremath{\mathbb{Q}(\sqrt{5})}}
\newcommand{\Gphi}{\ensuremath{\Gamma_{\varphi}}}
...
\begin{abstract}
We summarise four pillars---Ogg primes, Conway--Norton Hauptmoduln, Mayer-type
generators on $\Qp$, and Witten's $c{=}24$ holographic conjecture---and record how
$\Zp$ behaves on the discrete moonshine primes. We excerpt the truncated
$T_{\texttt{11A}}(q)$ series as certified in-code and document brute Apollonian norm
statistics that fail coefficient-level matching---a necessary negative result, since
counting functions (polynomial growth) cannot equal Hauptmoduln (exponential growth)
coefficient-by-coefficient. We then show that the generators of $\Gphi^{(2)}$
satisfy three group-theoretic conditions fully consistent with the structural
conjecture: (i) the minimal polynomial of each generator entry encodes level~$11$
directly; (ii) the cusp set $\{0,1,\infty\}$ of $\Gphi$ matches the cusps of
$\Gamma_{0}(11)$ prior to Atkin--Lehner identification; (iii) cyclic words carry
Galois-symmetric traces of the form $-11^{3}$, propagating level-$11$ arithmetic
multiplicatively into the geodesic spectrum.  Finally, we reframe the entire
landscape using the Hopf line bundle: the modular weight ladder is the Hopf
degree ladder over $X(1)\cong\mathbb{CP}^{1}$, the two spinor chiralities give
two strands (Monster-Hecke at integer weight; umbral-pariah at half-integer
weight), and the seam at weight one consists of CM forms.  At the prime $p=11$
five independent designations coincide---Ogg prime, $E_{11}$ conductor,
$\varphi^{5}+\psi^{5}=11$, $11\mid|\mathrm{O\,'\!Nan}|$, and Heegner number
$h(\mathbb Q(\sqrt{-11}))=1$---each separately certified by tests.  We restate
the conjecture in its correct form: $\Gphi$ is the real-quadratic geometry that
threads this single CM gate, and at the level-$\mathfrak{m}=(11)$ congruence
subgroup $\Gamma_{0}(\mathfrak{m})\subset\Gphi$, the Selberg zeta factors
through the base-change Hilbert L-function $L(E_{11}/\Qp,\,s) = L(s,E_{11})\cdot L(s,E_{11}\otimes\chi_{5})$.
A direct numerical test of the simpler statement
``$Z_{\Gphi}\sim L(E_{11}/\Qp,s)$ on the full Hilbert modular group'' falsifies
the naive form: the two decay scales are mismatched by the exact algebraic
ratio $\log\varphi^{20}/\log 5 = 20\log_{5}\varphi \approx 5.98$, where
$\varphi^{20}$ is the fundamental Galois-symmetric geodesic norm of $\Gphi$
and $5=\mathrm{disc}(\Zp)$ is the ramified prime dominating
$-L'/L(E_{11}/\Qp,s)$ at large $s$.  This ratio is itself an algebraic
invariant assembled from the three numbers that distinguish $\Qp$ from every
other real quadratic field; its integer component ($6$) is the
conjectured codimension $[\Gphi:\Gamma_{0}(\mathfrak{m})]$ at which the
true correspondence operates.  Existence of the Hilbert newform $f_{E_{11}}$
realising $L(E_{11}/\Qp,s)$ is guaranteed by modularity of elliptic curves
over $\Qp$ (Freitas--Le Hung--Siksek~2015).  Finally, we prove a minimality
lock-in: among all squarefree $d\le 200$, the construction
$(\Qp,\varphi,k=5,p=11)$ is the unique smallest-discriminant solution with
non-trivial Lucas recurrence reaching an Ogg prime with positive-genus modular
curve. Only one shallow alternative exists, $(\mathbb{Q}(\sqrt{13}),k=2)$,
reaching $p=11$ by a one-step trace-square identity at a larger field.
This converts the framework from an example into a classification.

Phase 2 then closes the loop quantitatively: we verify $\Gphi^{(2)}\subseteq\Gamma_{0}((11))$
by direct computation, prove the Hilbert-modular index
$[\Gphi:\Gamma_{0}((11))]=144=F_{12}$ and observe that this is the unique
non-trivial square Fibonacci number (Cohn 1964) and equals $(L_{5}+1)^{2}$
with $L_{5}=11$ our Ogg prime.  We then encode the base-change Hecke
eigenvalues $a_{\mathfrak p}(f_{E_{11}})$ at every prime $\le 97$, verify the
Ramanujan-Petersson bound throughout, and predict the cleanest direct test
$a_{\mathfrak p}=0$ at the smallest split prime $\mathfrak p\mid 19$.  Finally we
connect O'Nan moonshine (Duncan--Mertens--Ono 2017) to the level-11 structure:
the Shimura correspondence maps the identity-class O'Nan form $Z_{e}$ into the
$1$-dimensional space $S_{2}(\Gamma_{0}(11))^{\mathrm{new}}=\langle f_{E_{11}}\rangle$,
so the pariah strand and the Monster strand intersect at a single weight-$2$
newform, and that newform is $f_{E_{11}}$.

Phase~3 (the present revision) consolidates the proof material as a
\textbf{formal-theorems appendix} (§\ref{sec:formal-theorems}, Propositions
T1--T5), separates definitively-proven facts from open conjectures, and
adds four additional empirical fronts:
\textit{(a)} a Frobenius-trace test at the split primes
$\mathfrak p\mid\{19,29,31,41\}$ confirming the predicted
$a_{\mathfrak p}$-driven asymmetry in $\chi^{2}$ at finite truncation;
\textit{(b)} PARI/GP cross-validation of all 25 Hecke eigenvalues of $E_{11}$
at primes $\le 97$ (which revised eight previously-tabulated values);
\textit{(c)} the Hilbert-modular dimension formula
$\dim S_{(2,2)}(\Gamma_{0}((11)))=10$, of which exactly one form is the
base-change of $f_{E_{11}}$ and nine are genuinely-new Hilbert newforms;
and \textit{(d)} a tested-but-not-found search for a structural
identification of the pariah-count/spectral-depth agreement $6=\lceil
20\log_{5}\varphi\rceil$, which we downgrade to ``intriguing $0.4\%$
numerical coincidence.''  The headline conjecture, the spectral
identification of $L(E_{11}/\Qp,s)$ inside $Z_{\Gamma_{0}((11))}$, survives
every empirical falsifier in pure Python.

\paragraph{Reading guide.}  The paper is organised in three parts:
...
imagery; downgraded after Phase~3 from ``structural identity'' to
...
For $\Zp$ quadratic integer ring of $\disc(\Qp)=5$ we classify their splitting:
\begin{center}
...
\[
  \boxed{\;\frac{\log\varphi^{20}}{\log 5}\;=\;20\log_{5}\varphi\;\approx\;\sqrt{F_{12}}\;-\;\epsilon,
...
Pure $AdS_{3}$ gravity at Brown-Henneaux coupling $k=1$ was conjecturally tied
to monstrous symmetry~\cite{Witten07,GaiottoYin}: boundary theory should host
Moonshine symmetry at $c=24$. This aligns qualitatively with the substrate Lorentzian
tower $\mathrm{II}_{25,1}$ via Borcherds roots.
...
\section*{Companion engineering artefacts}
\begin{itemize}\itemsep0pt
